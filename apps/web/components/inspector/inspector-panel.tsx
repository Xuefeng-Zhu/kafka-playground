  ...
  return (
    <div className="flex h-full flex-col text-[#123047]">
      <header className="flex items-center justify-between border-b-[3px] border-teal-700 bg-[#fff7ed] px-5 py-4">
        <h2 className="kplay-section-title">{selectedNode ? "Topology Inspector" : "Message Inspector"}</h2>
        <button
          type="button"
          onClick={onClose}
          ...
          aria-label={selectedNode ? "Close topology inspector" : "Close message inspector"}
        >
...
function TopologyDetails({
  ...
  if (selectedNode.type === "producer") {
    return (
      <>
        <TopologyHeader title="Producer" detail="Source node for keyed messages" tone="teal" />
        <DetailSection
      ...
      <>
        ...
        <DetailSection
          ...
          rows={[
            ...
            ["Latest offsets", partitionRecord(snapshot.latestPartitionOffsets, snapshot.partitionCount)],
            ["Committed offsets", partitionRecord(snapshot.latestCommittedOffsets, snapshot.partitionCount)]
          ]}
        />
      </>
    );
  ...
  if (selectedNode.type === "partition") {
    ...
    const owner = snapshot.consumers.find((consumer) =>
      consumer.assignments.some((assignment) => assignment.partition === partition)
    );
    return (
      <>
        <TopologyHeader title={`Partition ${partition}`} detail="Ownership and offset state" tone={partition === 0 ? "sky" : "violet"} />
        <DetailSection
          title="Partition Metrics"
          rows={[
            ["Owner", owner?.consumerId ?? "Unassigned"],
            ["Latest offset", snapshot.latestPartitionOffsets[String(partition)] ?? "None"],
            ["Committed offset", snapshot.latestCommittedOffsets[String(partition)] ?? "None"],
            ["Observed messages", String(snapshot.messageCounts[String(partition)] ?? 0)],
            ["Recent lane messages", String(snapshot.recentMessages.filter((message) => message.partition === partition).length)]
          ]}
        />
      </>
    );
  ...
  const consumer = snapshot.consumers.find((item) => item.consumerId === selectedNode.consumerId);
  return (
    <>
      <TopologyHeader title={selectedNode.consumerId} detail="Consumer group member" tone={consumer?.assignments.length ? "green" : "amber"} />
      {consumer ? (
        <DetailSection
          title="Consumer Metrics"
          rows={[
            ["Status", consumer.status],
            ["Assignments", consumer.assignments.length ? consumer.assignments.map((assignment) => `P${assignment.partition}`).join(", ") : "Idle"],
        ...
        <div className="p-5 text-sm text-[#466778]">This consumer is no longer active in the current snapshot.</div>
      )}
    </>
  );
}
...
function TopologyHeader({
  ...
  title: string;
  detail: string;
  tone: "amber" | "green" | "sky" | "teal" | "violet";
}) {
  const toneClass = {
  ...
  return (
    <section className={`border-b-[3px] border-teal-700 p-5 ${toneClass}`}>
      <div className="text-sm font-semibold opacity-80">Selected topology node</div>
      ...
      <dl className="grid grid-cols-[120px_1fr] gap-x-4 gap-y-2 text-sm">
        {rows.map(([label, value]) => (
          <div key={label} className="contents">
            ...
            <dd className="min-w-0 break-words font-semibold text-[#123047]">{value}</dd>
          </div>
        ))}
      </dl>
    </section>
...
function keyStrategyLabel(snapshot: RunSnapshot) {
  if (snapshot.keyStrategy.type === "fixed") return `Fixed key: ${snapshot.keyStrategy.value}`;
  if (snapshot.keyStrategy.type === "round_robin_users") return "Three user IDs";
  if (snapshot.keyStrategy.type === "random_user") return "Random user ID";
  return "No key";
}

function partitionRecord(record: Record<string, string>, partitionCount: number) {
  return Array.from({ length: partitionCount }, (_, partition) => `P${partition}: ${record[String(partition)] ?? "none"}`).join(" / ");
}

function totalMessages(snapshot: RunSnapshot) {
  return Object.values(snapshot.messageCounts).reduce((sum, count) => sum + count, 0);
}
