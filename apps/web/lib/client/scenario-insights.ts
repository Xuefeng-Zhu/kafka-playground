...
export function deriveScenarioInsight(snapshot: RunSnapshot): ScenarioInsight {
  ...
  if (snapshot.scenarioId === "at-least-once-duplicates") {
    ...
    return {
      ...
      summary:
        "Watch the gap between processed work and committed offsets; duplicates are possible before commit.",
      metrics: [
        {
        ...
        {
          label: "Duplicate-risk records",
          value: String(duplicateRisks),
          tone: duplicateRisks > 0 ? "amber" : "sky",
        },
  ...
  if (snapshot.scenarioId === "retry-dead-letter-queues") {
    return {
      ...
      summary:
        "Every third simulated fulfillment request fails processing and is marked for retry/DLQ escalation.",
      metrics: [
        {
          label: "Failed",
          value: String(failed),
        ...
        },
        {
          label: "Retry topic",
          value:
            latestPayloadString(latest, "retryTopic") ?? "orders.retry.30s",
          tone: "amber",
        },
        {
          label: "DLQ",
  ...
  if (snapshot.scenarioId === "schema-evolution-karapace") {
    ...
    return {
      ...
      summary:
        "Compatible profile updates pass through; every fourth schema version simulates an incompatible contract.",
      metrics: [
        {
        ...
        },
        {
          ...
          tone: incompatible > 0 ? "rose" : "emerald",
        },
        {
          label: "Subject",
          value:
            latestPayloadString(latest, "schemaSubject") ?? "profile-value",
          tone: "sky",
        },
      ],
  ...
  if (snapshot.scenarioId === "transactional-producers") {
    return {
      ...
      summary:
        "Records carry transaction IDs, producer sequence numbers, and open/commit boundary hints.",
      metrics: [
        {
          label: "Transaction",
  ...
  if (snapshot.scenarioId === "event-replay-sourcing") {
    return {
      title: "Replay cursor",
      summary:
        "Domain events carry aggregate IDs and replay cursors so derived state can be rebuilt from the log.",
      metrics: [
        {
          label: "Aggregate",
        ...
        {
          label: "Event",
          value: latestPayloadString(latest, "eventName") ?? "-",
          tone: "violet",
        },
        {
          label: "Cursor",
  ...
  if (snapshot.scenarioId === "consumer-lag-backpressure") {
    return {
      ...
      summary:
        "Lag estimates grow when produced records outpace commits; add consumers or lower rate to recover.",
      metrics: [
        {
          label: "Estimated lag",
          value: String(lag),
        ...
        {
          label: "Latency",
          value: `${snapshot.processingLatencyMs} ms`,
  ...
  if (snapshot.scenarioId === "hot-partitions-key-skew") {
    ...
    return {
      ...
      summary:
        "The default hot key intentionally concentrates traffic so skew is visible in partition counts.",
      metrics: [
        {
          label: "Hot key",
          value:
  ...
  if (snapshot.scenarioId === "log-compaction-tombstones") {
    ...
    return {
      ...
      summary:
        "Repeated keys represent compacted updates; tombstones mark keys for eventual deletion.",
      metrics: [
        {
          label: "Tombstones",
        ...
        },
        {
          ...
          tone:
            latestPayloadString(latest, "operation") === "delete"
              ? "rose"
              : "emerald",
        },
        {
          label: "Compacted key",
          value: latestPayloadString(latest, "compactedKey") ?? "-",
          tone: "sky",
        },
      ],
  ...
  if (snapshot.scenarioId === "retention-data-loss") {
    ...
    return {
      ...
      summary:
        "Records outside the active window represent data that consumers may no longer be able to replay.",
      metrics: [
        {
        ...
        {
          label: "Replayable from",
          value: latestPayloadString(latest, "replayableUntilOffset") ?? "0",
          tone: "sky",
        },
        { label: "Committed", value: String(committed), tone: "emerald" },
      ],
  ...
  if (snapshot.scenarioId === "cooperative-rebalancing") {
    return {
      ...
      summary:
        "Add and stop members to watch incremental ownership movement under a cooperative-sticky model.",
      metrics: [
        {
          label: "Members",
  ...
  if (snapshot.scenarioId === "streams-joins-windows") {
    return {
      title: "Windowed join",
      summary:
        "Events carry stream names, join keys, and window boundaries to model stateful stream joins.",
      metrics: [
        {
          label: "Stream",
          value: latestPayloadString(latest, "streamName") ?? "-",
          tone: "violet",
        },
        {
          label: "Join key",
        ...
        {
          label: "Late arrivals",
          value: String(countPayload(messages, "lateArrival", true)),
          tone:
            countPayload(messages, "lateArrival", true) > 0
              ? "amber"
              : "emerald",
        },
      ],
  ...
  if (snapshot.scenarioId === "outbox-cdc") {
    return {
      title: "Outbox CDC",
      summary:
        "Records carry table operations, outbox IDs, and log sequence numbers for CDC publication.",
      metrics: [
        {
          label: "Table",
  ...
  if (snapshot.scenarioId === "acl-least-privilege") {
    ...
    return {
      ...
      summary:
        "Denied operations simulate Kafka authorization failures for principals with insufficient permissions.",
      metrics: [
        {
          label: "Denied",
          value: String(denied),
  ...
  return {
    ...
    summary:
      "Messages, partitions, consumer assignments, and commits show the core Kafka lifecycle.",
    metrics: [
