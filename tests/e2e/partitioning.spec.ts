...
test("demo scenario visualizes assignments, idle consumer, message details, and reset", async ({
  ...
  await page.evaluate(() =>
    window.localStorage.removeItem("kplay.runControls.expanded"),
  );
  await page.reload();
  ...
  const collapsedTimelineHeight = await timelineRegion.evaluate(
    (element) => element.getBoundingClientRect().height,
  );
  await expect(page.getByTestId("timeline-expand-toggle")).toHaveAttribute(
    "aria-expanded",
    "false",
  );
  ...
  await expect(
    page.getByRole("button", { name: "Collapse timeline" }),
  ).toBeVisible();
  const expandedTimelineHeight = await timelineRegion.evaluate(
    (element) => element.getBoundingClientRect().height,
  );
  ...
  await expect(page.getByTestId("run-settings-toggle")).toHaveAttribute(
    "aria-expanded",
    "false",
  );
  ...
  await expect(page.getByText("100%")).toBeVisible();
  await expect(topologyContent).toHaveCSS(
    "transform",
    "matrix(1, 0, 0, 1, 0, 0)",
  );
  ...
  await expect(page.getByTestId("ownership-connector-partition-0")).toHaveCount(
    1,
  );
...
test("non-primary scenarios are routable and startable", async ({ page }) => {
  ...
  await expect(
    page.getByRole("heading", { name: "Hot partitions and key skew" }),
  ).toBeVisible();
...
test("scenario navigation keeps the UI bound to the active run", async ({
  page,
}) => {
  await resetActiveRun(page);
  await page.goto("/scenarios/partitioning");
...
test("consumer crash remains visible and replays uncommitted work", async ({
  ...
  if ((await page.getByTestId("run-settings-panel").count()) === 0) {
    await page.getByTestId("run-settings-toggle").click();
  }
