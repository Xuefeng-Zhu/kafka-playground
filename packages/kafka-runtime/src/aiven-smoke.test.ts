...
import { createHeaders, createPlaygroundValue, createResourceNames } from "@kplay/scenario-engine";

const runIfAiven = process.env.RUN_AIVEN_E2E === "true" ? it : it.skip;

describe("optional real Aiven smoke", () => {
  runIfAiven("produces, consumes, commits, and deletes a unique topic", async () => {
    ...
    try {
      ...
      let resolveConsumed: (message: ConsumedMessage) => void = () => undefined;
      ...
      const consumed = new Promise<ConsumedMessage>((resolve, reject) => {
        ...
        setTimeout(() => reject(new Error("Timed out waiting for Aiven message.")), 30_000);
      });
