...
- **Explore** renders each core producer, topic/partitions, message, ownership, or consumer-group entity referenced by the scenario route exactly once, then combines those components with the converted scenario's projected causal nodes and labeled edges. It does not fabricate an irrelevant generic producer for a database-owned CDC route. At widths of 768 pixels and above, React Flow is dynamically imported only when Explore is opened. Below 768 pixels, the same ranked projection becomes a semantic list and the page owns scrolling.

Explore exposes **Simulated runtime topology** or **Observed broker topology** as its accessible region name without spending a full content row on a repeated banner. The global runtime mode plus provenance on every node and edge keeps the distinction visible. Remote and Aiven runs remain core-only because no scenario extension may be inferred from broker state. An allowlisted scenario whose authoritative frame is temporarily unavailable also falls back to core-only Explore instead of the old overlay. Legacy Insights and overlays remain confined to truly unconverted scenario IDs during the migration soak.

The header view switch uses tab semantics. Converted scenario start pages keep the saved/default switch visible but disabled, with an accessible prompt to start a run; this prevents header layout shift without exposing empty panels. Switching preserves run state, experiment progress, checkpoint answers, `FocusRef`, inspector state, and the active Explore dock tab. Remote, Aiven, and legacy runs force Explore without overwriting the saved demo preference. Re-entering desktop Explore frames the projected topology at a readable 100% home zoom; learners can zoom out to 50% for an overview and pan to reach the remainder. Pan and zoom are intentionally transient.
